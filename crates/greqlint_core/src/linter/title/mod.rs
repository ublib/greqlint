use crate::{normalizer::Issue, parser, regex_generator::gen_regex};
...
pub fn lint_title(schema: &Schema, issues: &Vec<Issue>, title: &str) -> Result<(), String> {
...
mod tests {
    ...
    fn test_lint_title() {
        ...
        let issues = vec![Issue { number: 42 }];
        assert_eq!(lint_title(&schema, &issues, "feat(scope): message"), Ok(()));
        assert_ne!(
            lint_title(&schema, &issues, "aaa(scope): message message"),
            Ok(())
        );
    }
}
